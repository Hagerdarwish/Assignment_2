...
import com.example.assignment2.databinding.ActivityHomeBinding

class HomeActivity : AppCompatActivity() {
  lateinit var binding:ActivityHomeBinding
 var list: ArrayList<Course_Item> = ArrayList()
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding=ActivityHomeBinding.inflate(layoutInflater)
        initList()
        binding.buttonIos.setOnClickListener{
            startDetailsScreen(list[1])
        }
        binding.button3.setOnClickListener{
            startDetailsScreen(list[0])
        }
        binding.buttonWeb.setOnClickListener{
            startDetailsScreen(list[2])
                ...
                list.add(
                    Course_Item(
                        "Part 1 ( Java SE)\n" +
                                ...
                                "• one signal push notifications SDK", R.drawable.android)
                )
            }
            if (i==2){
                list.add(Course_Item("OOP refreshment\n" +
                        "• Introduction\n" +
                        ...
                        "• Creating final project depends on attendees needs", R.drawable.ios))
            }
            if (i==3){
                list.add(
                    Course_Item(
                        "HTML\t\n" +
                                ...
                                "•agile\n" +
                                "•Scrum\n" +
                                "•Software development process", R.drawable.fullstack)
                )
            }

        }
    }
    private fun startDetailsScreen(courseItem: Course_Item) {
        val intent = Intent(this, DetailsActivity::class.java)
        intent.putExtra("data", courseItem.data)
